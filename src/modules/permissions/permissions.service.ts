...
export class PermissionsService {
  ...
  // Assignments
  async assignPermissionsToGroup(groupId: number, assignPermissionsDto: AssignPermissionsDto): Promise<Group> {
  ...
  async assignUsersToGroup(groupId: number, assignUsersDto: AssignUsersDto): Promise<Group> {
    const group = await this.findOneGroup(groupId);
  ...
  async assignDirectPermissionsToUser(userId: number, assignPermissionsDto: AssignPermissionsDto): Promise<User> {
    if (isNaN(userId)) {
  ...
  async updateGroupPermissionStatus(
    ...
    if (!groupPermission) {
      throw new NotFoundException('Vínculo entre grupo e permissão não encontrado');
    }
  ...
  async updateUserPermissionStatus(
    ...
    if (!userPermission) {
      throw new NotFoundException('Vínculo entre usuário e permissão não encontrado');
    }
