...
export class VacationService {
  ...
  async findAll(query: VacationQueryDto) {
    ...
    if (query.startDate) {
      conditions.push(`V.DATA_INICIO >= TO_DATE(:${values.length + 1}, 'YYYY-MM-DD')`);
      values.push(query.startDate);
    }

    if (query.endDate) {
      conditions.push(`V.DATA_FIM <= TO_DATE(:${values.length + 1}, 'YYYY-MM-DD')`);
    ...
    for (const [key, column] of Object.entries(mapping)) {
      if (dto[key] !== undefined) {
        if (key === 'startDate' || key === 'endDate') {
          fields.push(`${column} = TO_DATE(:${placeholderIndex++}, 'YYYY-MM-DD')`);
        } else {
  ...
  async remove(id: number) {
    try {
      await this.dataSource.query('DELETE FROM GP_FERIAS WHERE CODIGO = :1', [id]);
    } catch (error) {
